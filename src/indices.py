...
spindex = SpectralIndices(
    SpectralIndices=dict(
        ...
        kEVI=SpectralIndex(
            ...
            formula='g * (kNN - 2*kNR + kRR) / ((C2 ** 2)*kBB - 2*C2*kBL - 2*C2*kNB - 2*C1*C2*kBR + kLL + 2*kNL + 2*C1*kRL + kNN + 2*C1*kNR + (C1 ** 2)*kRR)',
        ...
        kVARI=SpectralIndex(
            ...
            formula='(kGG - 2*kGR + kRR) / (kBB - 2*kGB - 2*kRB + kGG + 2*kGR + kRR)',
            reference='https://doi.org/10.1126/sciadv.abc7447',
            type='kernel',
            date_of_addition='2021-05-10',
            contributor="https://github.com/davemlz"
